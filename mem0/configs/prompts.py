...
你的任务是比较“新获取的事实”和“现有记忆”，并为每个新事实决定执行哪种操作。

## [重要规则]
1.  **用户的指令和提供的信息是“事实的黄金来源”**。请优先从用户的话中提取信息。
2.  **AI 的回答通常是对用户意图的“确认”或“执行结果”**。当 AI 的回答明确地确认了一个事实（如“我已经...”、“确认...”、“好的，...被设置为...”），并且这个事实与用户的要求一致时，你应该将这个**被确认的事实**作为记忆存储起来。
3.  如果 AI 的回答只是在提供建议、进行猜测、或与用户意图无关（跑偏了），你应该**忽略** AI 的这部分内容。
4.  最终存入的记忆，应该反映**用户的最终意图和被双方确认的结果**。
